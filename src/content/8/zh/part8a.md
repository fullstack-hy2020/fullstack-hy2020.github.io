...
让我们用当今领先的图书馆[ Apollo-server ]( https://www.apollographql.com/docs/Apollo-server/ 服务器)实现一个 graphql 服务器。

Create a new npm-project with _npm init_ and install the required dependencies.
使用 npminit 创建一个新的 npm-project，并安装所需的依赖项。
...
第二个参数是一个对象，它包含服务器的[解析器]( https://www.apollographql.com/docs/apollo-server/essentials/data.html#resolver-map )。 这些代码定义了 / i GraphQL 查询的响应方式。
...
当 Apollo-server 在开发模式(node filename.js)下运行时，它会启动一个[ GraphQL-playground ]( https://www.apollographql.com/docs/Apollo-server/testing/GraphQL-playground/ 文件)来寻址[ http://localhost:4000/graphql 文件]( http://localhost:4000/graphql 文件)。 这对于开发人员非常有用，可用于对服务器进行查询。
...
事实上，所有的解析器函数都是给定的[4个参数]( https://www.apollographql.com/docs/graphql-tools/resolvers.html#resolver-function-signature )。 使用 JavaScript 时，如果不需要参数，那么就不必定义它们。 我们将在本部分后面使用解析器的第一个和第三个参数。
...
现在的响应是一个 person 对象，它包含 / i 一个地址对象。
...
从[这个文件]( https://github.com/fullstack-hy2020/misc/blob/master/library-backend.js )开始，记住 npminit 并安装依赖项！
