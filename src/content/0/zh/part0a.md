...
本课程还包含一些其他的议题，包括如何调试你的应用、如何配置、如何和管理运行时环境以及如何使用 NoSQL 数据库。
...
参与者需要具有良好的编程技能，基本的网络编程和数据库知识，能够掌握 Git 版本管理的基本使用。 你当然还要有坚持不懈的精神、独立解决问题和搜寻信息的能力。

Previous knowledge of JavaScript or other course topics is not required.

此外，不需要有 JavaScript 或其他课程的相关知识。

### Course material 课程材料

The course material is meant to be read one part at a time, reading each part all the way through before moving on to the next one.课程是这么设计的，你应该一次只读一章节，并在继续学习下一章节之前，把之前的章节都学透彻。
...
课程是将一个示例应用从小做大的过程，也就是从章节到章节地对代码进行改进。 因此可能最好的学习方法是紧跟代码的修改进度，并独立地对代码进行小修改。 每一章节、每个步骤的示例代码都可以在 GitHub 上找到。

### Taking the course 参加课程
...
从第 n 章跳转到第 n + 1 章之前，最好对第 n 章的主题有足够的了解。 在教学方法论上，本课程使用了 [掌握学习法（mastery learning）](https://en.wikipedia.org/wiki/Mastery_learning)，其精髓就是让你在完成上一章节足够的练习之后再进入下一章节。
...
你至少要完成没有带星号(\*)的练习。 带星号的练习虽然对标你的期末成绩，但跳过它们并不妨碍你做下一章节的必修练习。
...
学分的数量和成绩是由提交的练习的数量决定的，也就是说没有星号的练习也会计入你的最终成绩。
...
完成足够的练习后，成绩通过，你可以从 [提交系统](https://studies.cs.helsinki.fi/stats/courses/fullstackopen)下载课程证书。 获得课程证书，不需要芬兰社保号码。
...
想要获得大学学分来的话，3 个学分的课程也要求通过课程考试。 考试是在 Moodle 上完成的。 一旦[注册](/en/part0/general_info/#sign-up-for-the-exam)，就可以通过[开放大学的课程页面](https://courses.helsinki.fi/en/aytkt21009en/131840261)访问 Moodle 系统。
...
| exercises 练习数 | credits 学分数 | grade 成绩 |
| ---------------- | :------------: | :--------: |
| 138              |       7        |     5      |
| 127              |       6        |     5      |
| 116              |       5        |     5      |
...
你必须提交 5 学分的足够的练习数才能参加考试。 实践说明，在刚提交了满足数量的练习之后就立即参加课程考试是十分不明智的。 5-9 学分的考试也是一样，不计入你的成绩。
...
通过提交第 8 章节 GraphQL 的至少 22 / 26 个练习，就可以获得一个额外的学分。 第 8 章节可以在第 5 章节之后的任何时候完成，因为其内容独立于第 6 章节和第 7 章节。
...
第 8 和第 9 章节的练习不计入 5-7 学分课程的成绩。

### How to get your credits 如何获得学分
...
即使你没有因为考试而注册公开大学，只要你完成了足够的练习数量，获得了通过成绩，你就可以从 [提交系统](https://studies.cs.helsinki.fi/stats/courses/fullstackopen) 中下载课程的结业证书。如果只想要获得结业证书，你不需要提供芬兰社保账号。

### Expanding a previously completed course 拓展之前完成的课程

If you have already taken the course either as a MOOC or as a University Course, you can now expand your course.

如果你已经参与过这门课程，无论是大学课程还是公开课，你都可以基于这门课来进行拓展。

#### Expanding Full stack open 2019 拓展全栈公开课 2019

You can just pick up where you left! If you wish to resubmit a whole part, please contact the course personel.

你可以续接你上次学习的进度！ 如果您希望重新提交整个课程内容，请与课程负责人联系。

#### Expanding other course instance 拓展其他课程
...
你可以用之前已经提交的部分内容来代替本课程的内容。 例如，如果你之前已经完成了 3 个学分的课程，你可以用之前完成的内容来替换这门课程的 0-3 学分的内容。但你只能按顺序替换章节，例如你不能学完了第 1 章节，但想要替换第 0 和第 2 章节的内容。
...
练习是通过 GitHub 提交的，并按照[提交系统](https://studies.cs.helsinki.fi/stats/courses/fullstackopen)那样标记练习。
...
如果要将来自不同章节的练习提交到同一个仓库，那么可以使用一些合理的命名规范来命名自己的目录。 您当然可以为每个章节创建一个新的仓库。 如果您使用私有仓库，需要添加 mluukkai 作为 collaborator。
...
不要使用 nano、 Notepad 或 Gedit ， NetBeans 对于 web 开发来说也不是很好。 与 Visual Studio Code 相比，它也显得有些重。
...
同时记得安装 Node.js。 课程材料已经在 10.18 版本中验证过了，所以不要安装任何更老的版本。 安装说明在 [这里](https://nodejs.org/en/download/package-manager/)。
...
### Typos in the material 教材中的拼写错误
...
如果你发现材料中有一个拼写错误，或者有些东西表达不清楚，又或者只是简单的语法错误，请在 https://github.com/fullstack-hy2020/fullstack-hy2020.github.io 向仓库中的教材内容提出 pull request。 例如，这个页面的 markdown 代码可以在 https://github.com/fullstack-hy2020/fullstack-hy2020.github.io/edit/source/src/content/0/en/part0a.md 的仓库中找到
...
在每个章节的底部是一个内容建议的链接。 通过点击链接，就以编辑该页面的源码啦~
