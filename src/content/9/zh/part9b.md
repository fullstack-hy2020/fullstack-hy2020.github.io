...
编写一个函数<i>calculateExercises</i>，计算<i>每天运动小时</i> 的平均时间，并将其与每天运动小时的<i>目标数量</i> 进行比较，然后返回一个包含如下值的对象:
...
Web 应用应该在生产模式下以命令<i>npm start</i> 启动，而<i>npm 在开发模式下运行 dev</i>，应该使用<i>ts-node-dev</i> 来运行应用。
...
添加一个端点到您的应用的锻炼计算器。 它应该通过对端点 i 使用 HTTP POST 请求来使用请求体中的输入
