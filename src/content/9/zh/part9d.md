...
如果我们犯了这些错误中的任何一个，打字稿可以帮助我们马上在我们的编辑器中捕捉这些错误。 如果我们不使用打字稿，我们将不得不在以后的测试中捕捉这些错误，并且我们可能被迫进行一些乏味的调试，以找出错误的原因。

But let that be enough of reasoning for now, and let's start getting our hands dirty!
但是现在就让这些推理到此为止吧，让我们开始把手弄脏吧！
...
[ create-react-app ]( https://create-react-app.dev )通过在初始化脚本中添加一个额外的 i template / i 参数，支持使用 TypeScript 创建应用程序的方法。 因此，为了使用 TypeScript 创建一个 Create React App，运行以下命令:
...
在运行命令之后，你应该有一个完整的基本反应应用程序，它使用打字稿。 只需在应用程序根文件夹中运行 i npm start / i 即可启动该应用程序。
...
如果你浏览这些文件和文件夹，你会发现这个应用程序和你用纯 JavaScript 初始化的应用程序没什么不同。 基本上，唯一的区别是 i。 我和我。 Jsx / i 文件现在改名为 i。 Ts / i 和我。 Tsx / i 文件，它们包含一些类型注释，根文件夹也包含一个 i tsconfig.json / i 文件。
...
现在，让我们看看为我们创建的 i tsconfig.json / i 文件。 文件中的所有内容都应该差不多正常，只是目前的配置还允许编译 JavaScript 文件，因为 i allowJs / i 被设置为 i true / i。 如果你需要混合使用打字脚本和 JavaScript (例如，如果你正在将一个 JavaScript 项目转换成打字脚本或者其他原因) ，那么这样做是可以的，但是我们希望我们的应用程序是纯粹的打字脚本，所以让我们把这个设置改为 i false / i。
...
之前我们添加了 eslint 来帮助我们在后端实施编码风格，所以让我们对这个应用程序做同样的事情。 我们不需要安装任何依赖项，因为 create-react-app 已经解决了这个问题。
...
因为基本上所有 React 组件都返回一个 i JSX.Element / i 类型或 i null / i 类型，所以我们通过禁用 rule [ explicit-function-return-type ](explicit-function-return-type)来稍微放松默认的链接规则，这样我们就不需要在所有地方显式写出函数返回类型 https://github.com/typescript-eslint/typescript-eslint/blob/master/packages/eslint-plugin/docs/rules/explicit-function-return-type.md。
...
现在我们需要让我们的 linting 脚本来解析 i * 。 Tsx / i 文件也是如此，它们是与 react 的 JSX 文件等价的打印脚本。 我们可以通过改变 i 中的 lint 命令来做到这一点。 包裹 json / i 到以下地址:
...
如果我们现在运行 i npm 运行 lint / i，我们不应该得到任何错误。 实际上，错误并不重要，因为我们不需要文件 i servicedworker.ts / i，所以最好将其删除。
...
在这个示例中，我们有一个名为 i Welcome / i 的组件，我们希望向它传递一个 i name / i prop，然后将其渲染。 我们知道 i name / i prop 应该是一个字符串，我们使用[ prop-types ](prop-types)包[ part 5](/ en / part5 / props children and proptypes # prop-types)中引入的[ prop-types ](prop-types) ，可以接收关于部件所需道具类型的提示，以及无效道具类型的警告。
...
当使用 TypeScript 和 React 组件时，类型注释看起来与其他 TypeScript 代码有点不同。 我们基本上为组件变量添加一个类型，而不是函数和它的道具。 做出反应。 Functioncomponent 是一种所谓的[泛型](泛型)类型，您可以将类型作为参数传递给它，然后它在最后的类型中使用这个 https://www.typescriptlang.org/docs/handbook/generics.html。
...
在这里您可以看到 i props / i 是 i PropsWithChildren / i 类型，这也是一个泛型类型，i p / i 被传递给它。 类型 i PropsWithChildren / i 依次是 i p / i 和类型 i { children？ : ReactNode } / i.
...
好吧，这很复杂(或者说是这样吗?) . 基本上，我们现在需要知道的是，我们可以定义一个传递给 FunctionComponent 的类型，然后组件的 i props / i 包含已定义的类型和组件的 i children / i。
...
现在，让我们回到我们的代码示例，看看如何在打字稿中为 i Welcome / i 组件的 props 定义类型。
...
现在我们的编辑器知道 i name / i prop 是一个字符串，但是由于某些原因 eslint 还不满足，并且警告说在 props validation / i 中缺少 i‘ name。 这是因为 react linting 规则期望 propTypes 为所有道具定义，因为它没有意识到我们已经在使用 TypeScript 为道具定义类型。
...
使用 TypeScript 创建一个新的 Create React App，像我们刚才做的那样为项目设置 esint。
...
这个练习和你在[第1部分](/ en / part1 / javascript # exercises-1-3-1-5)中已经做过的类似，只是添加了打印脚本和一些调整。 首先将 i index.tsx / i 的内容修改为:
...
整个应用程序现在在一个组件中，这不是我们想要的，因此重构代码，使其由三个新组件组成: i Header / i、 i Content / i 和 i Total / i。 所有数据仍然保存在 i App / i 组件中，该组件将所有必要的数据作为道具传递给每个组件。 一定要为组件的道具添加类型声明！
...
在前面的练习中，我们有一个课程的三个部分，其中所有部分都具有相同的属性 i name / i 和 i exercecont / i。 但是，如果我们需要为每个部分添加额外的属性，而每个部分又需要不同的额外属性，那么这看起来像是代码吗？ 让我们考虑下面的例子:
...
在上面的例子中，我们为每个课程部分添加了一些额外的属性。 属性 i name / i 和 i exerciseCount / i 对于每个部分都是相同的，第一部分和第三部分共享一个名为 i description / i 的属性，还有一些附加属性对于第二部分和第三部分是不同的。
...
让我们假设我们的应用程序一直在增长，我们需要在我们的代码中传递不同的课程部分，除此之外，还有额外的属性和额外的课程部分添加到混合中。 那么我们如何知道我们的代码能够以正确的方式处理所有不同类型的数据，以及我们不会忘记在某些页面上呈现新的课程部分？ 这就是打字稿能够特别有用的地方！
...
接下来，我们将创建一个所有这些类型的类型[ union ]( http://www.typescriptlang.org/docs/handbook/advanced-types.html#union-types 类型) ，我们可以用它来为数组定义一个类型，它应该接受以下任何一个课程部分类型:
...
那么新的类型呢？ 如果我们要添加一个新的课程部分，如果我们还没有在代码中实现这种类型的处理，那不是很好吗？ 对于上面的示例，新类型将转到 i default / i 块，并且不会为新类型打印任何内容。 有时这当然是完全可以接受的，例如，如果您只想处理类型联合的特定(并非全部)情况，但在大多数情况下，建议分别处理所有变体。
...
对于打字稿，我们可以使用一个名为 i 穷举类型检查 / i 的方法。 它的基本原理是，如果遇到意外值，我们调用一个函数，该函数接受类型为[ never ]( https://www.typescriptlang.org/docs/handbook/basic-types.html#never )的值，并且返回类型为 i never / i。
...
现在我们知道了 i CoursePartOne / i 和 i CoursePartThree / 这两个接口不仅共享基本属性，而且共享一个名为 i description / i 的属性，这是两个接口中的字符串。
...
您的第一个任务是声明一个新界面，其中包括 i description / i 属性并扩展 i / CoursePartBase / i 界面。 然后修改代码，以便可以从 i CoursePartOne / i 和 i CoursePartThree / i 中删除 i description / i 属性，而不会出现任何错误。
...
创建一个组件 i Part / i，它呈现每种类型的课程部分的所有属性。 使用基于开关大小写的详尽类型检查！ 在组件 i Content / i 中使用新组件。
...
在大多数情况下，您可以使用 i type / i 或 i interface / i，这是您喜欢的任何语法，但仍有一些事情需要记住。 例如，如果您使用相同的名称定义多个接口，它们将导致一个合并的接口，而如果您尝试使用相同的名称创建多个类型，它将导致一个错误声明一个相同的名称已经声明。
...
打字稿文档[建议使用接口]( http://www.typescriptlang.org/docs/handbook/advanced-types.html#interfaces-vs-type-aliases )在大多数情况下。
...
当第一次深入到代码库中时，最好能够对项目的约定和结构有一个全面的了解。 您可以从存储库根目录中的 i README.md / i 开始进行研究。 通常，它包含应用程序和使用它所需要的需求的简要描述，以及如何启动它进行开发。 如果这个没有出现，或者有人节省了时间，把它留作存根，你可以在 i package.json / i 中看一眼。 启动应用程序并围绕它进行单击，同时验证您拥有一个功能性开发环境，这总是一个好主意。
...
您还可以浏览文件夹结构，以了解应用程序的功能和 / 或所使用的体系结构。 这一点并不总是很清楚，开发人员可能已经选择了一种组织代码的方法，而这种方法对您来说可能并不熟悉。 在本部分的其余部分中使用的[样本项目]( https://github.com/fullstack-hy2020/patientor )是有组织的。 你可以看到应用程序有哪些不同的页面和一些通用组件，例如。 情态动词和状态。 请记住，所代表的特性可能有不同的范围，例如。 模态是可见的 UI 级组件，而状态可以与业务逻辑相媲美，并保持数据组织在引擎盖下供应用程序的其余部分使用。
...
提供的类型可以告诉你需要什么样的数据结构、函数、组件和状态。 您可以尝试寻找 i typess.ts / i 或类似的东西来开始。 Vscode 是一个很大的帮助，仅仅突出显示变量和参数就可以提供很多洞察力。 这自然取决于在项目中如何使用类型。
...
如果项目有单元测试、集成测试或端到端测试，那么阅读这些测试很可能是有益的。 在重构或为应用程序创建新特性时，测试用例是最重要的工具。 您需要确保在处理代码时不会破坏任何现有的特性。 在对代码进行更改时，TypeScript 还可以为您提供参数和返回类型的指导。
...
记住，阅读代码本身就是一项技能，如果你在第一次阅读时不理解代码，那么就放松一下。 在整个开发周期中，代码可能有很多角落情况和添加的逻辑。 很难想象之前的开发者一直在与什么样的麻烦作斗争。 把它想象成[树木的年轮]( https://en.wikipedia.org/wiki/dendrochronology#growth_rings )。 理解所有这些内容需要深入研究代码和业务领域的需求。 你阅读的代码越多，你就会越熟悉它。 您将阅读比生成更多的代码。
...
如果一切顺利，您将看到一个病人列表页面，该页面从我们的后端获取一个病人列表，并在一个简单的表中呈现该列表，还有一个按钮用于在后端创建新的病人。 由于我们使用的不是数据库，而是模拟数据，因此数据不会持久，即关闭后端忘记所有添加的数据。 用户界面显然不是这个应用程序开发者的强项，所以让我们暂时忽略这个用户界面。
...
在我们验证了一切正常之后，我们开始研究代码。 所有有趣的内容都位于 i src / i 文件夹中。 为了方便起见，还有一个针对应用程序中使用的基本类型的现成 i types.ts / i 文件，您必须在练习中扩展或重构这个文件。
...
原则上可以在后端和前端中使用相同的类型，但是前端通常具有不同的数据结构和数据的用例，从而导致类型不同。 例如，前端具有一个状态，并且可能希望将数据保存在对象或映射中，而不是由具有不同键的后端给出的数组。 前端可能也不需要后端提供的所有字段，它可能需要添加一些用于呈现的新字段。
...
让我们更仔细地研究一下状态处理，因为很多事情似乎都发生在引擎盖下面，而且它与本课程到目前为止使用的方法有点不同。
...
状态管理是使用 React Hooks [ useContext ]( https://reactjs.org/docs/Hooks-reference.html#useContext )和[ useReducer ]( https://reactjs.org/docs/Hooks-reference.html#useReducer )构建的。 这是一个创建有状态应用程序的非常可行的选择，当我们知道我们的应用程序会相当小，我们不想使用 i redux / i 或其他库来进行状态管理时。 有很多好的素材。 [本文]( https://medium.com/@seantheurgel/react-hooks-as-state-management-usecontext-useeffect-usereducer-a75472a862fe )描述了这种方法。
...
在这个应用程序中采取的方法使用了 React [ context ]( https://reactjs.org/docs/context.html 文档) ，根据文档
...
I... 被设计用于为 React 组件树共享可被视为“全局”的数据，例如当前经过身份验证的用户、主题或首选语言。 我
...
在我们的示例中，“全局”共享数据是应用程序状态 i，/ i 是用于更改数据的分派函数。 在某种程度上，这种方法的工作原理很像我们在[ part 6](/ en / part6)中使用的基于 redux 的状态管理，但是它的重量更轻，因为它不需要使用任何外部库。 这一部分假设您至少熟悉 redux 的工作方式，例如，您至少应该包括第6部分的[第一部分](/ en / part6 / flux 架构和 redux)。
...
应用程序的[上下文]( https://reactjs.org/docs/context.html )有一个用于更改应用程序状态的应用程序状态和调度程序的元组。 输入的应用程序状态如下:
...
该状态是一个具有一个 key i patients / i 的对象，该对象具有一个[ dictionary ]( https://www.typescriptlang.org/docs/handbook/advanced-types.html#index-types-and-index-signatures ) ，或者简单地将一个具有字符串键的对象和一个 i Patient / i 对象作为值。 索引只能是 i string / i 或 i number / i，因为您可以使用它们访问对象值。 这强制状态以我们想要的方式符合，并防止开发人员滥用状态。
...
这是使用 TypeScript 帮助控制应用程序状态的一种方法。 我们知道，如果状态是 i type: “ Authenticated” / i，我们将有一个 i currentUser / i 字段。
...
这与从[ useReducer ]( https://reactjs.org/docs/hooks-reference.html#useReducer )接收的参数相匹配。 钩。 Dispatch 函数使用 i Action / i 作为参数对象，i 在 i reducer.ts / i 中定义，与定义动作对返回新状态的状态的实际 reducer 函数一起使用。
...
我们的状态管理方法的主要原则是通过上下文将状态传递给组件，并使用还原器修改状态。
...
在 i state.ts / i 文件中发生了许多事情，它们负责设置 contex。 它的主要组成部分是使用[ useReducer ]( https://reactjs.org/docs/hooks-reference.html#useReducer )钩子来创建 state 和 dispatch-function，并将它们传递给[ context provider ]( https://reactjs.org/docs/context.html#contextprovider ) :
...
不要担心，如果这看起来令人困惑，它肯定是直到你研究了[上下文文档]( https://reactjs.org/docs/context.html 文档)和它在[国家管理]( https://medium.com/@seantheurgel/react-hooks-as-state-management-usecontext-useeffect-usereducer-a75472a862fe 文档)中的使用。 做这些练习，你不需要完全了解这一切！
...
实际上，当您开始使用一个现有的应用程序时，在开始的时候您并不完全了解背后发生的事情，这是非常典型的。 如果应用程序结构合理，你可以相信，如果你正在仔细修改，应用程序仍然工作，尽管事实上你不了解所有的内部。 随着时间的推移，您可以掌握更多不熟悉的部分，但这不会发生在一夜之间，当工作与大型代码库。
...
让我们来看一下 i patientlistpage / index。 Ts / i，因为您可以从中获得灵感，帮助您从后端获取数据并更新应用程序的状态。 我 PatientListPage / i 使用我们的自定义钩子来注入 state 和 dispatcher 来更新状态。 当我们列出患者名单时，我们只需要从国家那里解构患者 / i 属性:
...
我们还使用 i useState / i hook 创建的应用程序状态来管理模态可见性和表单错误处理:
...
我们给出了应用于实际状态的 i useState / i hook 类型参数。 所以 i modalOpen / i 是一个 i boolean / i，而 i error / i 的类型分别是 i string | undefined / i。 这两个由 i / useState / i 钩子返回的 set 函数都是根据给定的类型参数只接受参数的函数，例如。 I setModalOpen / i 函数的确切类型是 i React。 调度反应。 布尔 / i。
...
Frontend 的类型基于你在开发前面部分的后端时所创建的类型。
...
由于我们的应用程序非常小，我们将通过调用 i useStateValue / i hook 提供的 i dispatch / i 函数来更新状态。 编译器有助于确保我们根据预定义类型字符串和有效负载的 i Action / i 类型分派操作:
...
我们将很快为我们的应用程序添加新类型 i Entry / i，它代表一个轻量级的病人日志条目。 它由杂志正文组成。 I 描述 / i，创建日期，关于创建它的专家的信息和可能的诊断代码。 诊断代码映射到从 i / api / Diagnosis / i 端点返回的 ICD-10代码。 我们天真的实现将是病人有一个条目数组。
...
从在提供的练习中创建的 enpoint 中获取数据。 从后端获取患者信息之后，将获取的信息添加到应用程序状态。 如果信息已经处于应用程序状态，也就是说，如果你多次访问同一个病人的页面，不要获取信息。
...
应用程序使用[ Semantic UI React ]( https://React.Semantic-UI.com/ )来进行样式化，这与[ React Bootstrap ]( https://React-Bootstrap.github.io/ )和[ MaterialUI ]( https://material-UI.com/ )非常相似，我们在[ part 7]中已经介绍过了(/ en / part7 / more about styles)。 你也可以在新的组件上使用它，但这取决于你，因为主要的焦点现在是打字稿。

The Application also uses [react router](https://reacttraining.com/react-router/web/guides/quick-start) 
应用程序还使用了[反应路由器]( https://reacttraining.com/react-router/web/guides/quick-start 路由器)
...
控制前端可见的视图。 如果你还没有掌握路由器的工作原理，你可能需要查看一下[ part 7](/ en / part7 / react router)。
...
当分派动作时，我们正在代码中随处创建 i action / i 对象，例如，component i App / i 具有以下特性:
...
在[练习9.12. ]中 (/ en / part9 / typing the express app # exercises-9-12-9-13)我们实现了一个用于获取诊断的端点，但是我们仍然根本没有使用该端点。 既然我们现在有一个页面可以查看单个病人的详细信息，那么最好将我们的数据扩展一点; 让我们向病人数据中添加可能的 i Entry / i 数据，这样每个病人都可以有包含可能诊断的医疗条目。
...
我们马上可以看到前几个字段是相同的，但是第一个字段只有 i discharge / i 字段，第二个字段只有 i employerName / i and i sickLeave / i。 因此，所有的条目似乎都有一些共同点，但有些是条目特定的。
...
当通过 i type / i 字段查看条目时，我们可以看到实际上有三种不同的条目: i OccupationalHealthcare / i、 i Hospital / i 和 i HealthCheck / i。 这表明需要三种不同类型的类型，但由于它们似乎都有一些共同点，我们可能只是想创建一个基本条目接口，我们可以用每种类型的不同字段进行扩展。
...
您应该使用基于开关大小写的渲染和 i 详尽的类型检查 / i，这样就不会忘记任何大小写。
...
在 React 中，表单处理有时会非常麻烦。 这就是为什么我们决定在我们的应用程序中使用[ Formik ]( https://jaredpalmer.com/Formik/docs/overview  / 病人表单)包。 以下是对 Formiks 的文档的一个小介绍:
...
在这里我们可以看到组件需要两个道具，i onSubmit / i 和 i onCancel / i。 两者都是返回 i void / i 的回调函数。 作为参数 i onSubmit / i 应该接收 i PatientFormValues / i 类型的对象，以便回调可以处理我们的表单值。
...
在创建 i AddPatientForm / i 函数组件时，您可以看到我们使用 bound i Props / i 类型作为组件的道具，并从这些道具中析构 i onSubmit / i 和 i onCancel / i。
...
让我们仔细研究一下 i SelectField / i 及其周围的类型。 首先，我们为每个选项对象创建一个泛型类型，其中包含该值的一个值和一个标签。 这些是我们希望在表单中的 select 字段中允许的选项对象。 因为我们只允许不同性别的选项，所以我们将 i value / i 设置为 i Gender / i 类型。
...
接下来查看类型 i SelectFieldProps / i。 它定义了我们的 i SelectField / i 组件的道具的类型。 在这里您可以看到选项是 i GenderOption / i 类型的数组。
...
函数组件 i SelectField / i 本身非常直接。 它呈现标签、选择元素和所有给定的选项元素，或者实际上它们的标签和值。
...
该组件呈现一个 SemanticUI [ Form。 字段]( https://react.semantic-ui.com/collections/form/ )和一个 Formik [字段]( https://jaredpalmer.com/Formik/docs/api/Field ) ，i name / i 和 i 占位符 / i 作为道具。
...
注意，输入的可能错误消息是通过 Fromik 组件[ ErrorMessage ]( https://jaredpalmer.com/formik/docs/api/ErrorMessage )呈现的，它可以处理所有事情，而不需要指定它应该做什么。
...
现在回到 i AddPatientForm.tsx / i 中的实际表单组件。 函数组件 i AddPatientForm / i 呈现一个[ Formik 组件]( https://jaredpalmer.com/Formik/docs/api/Formik 组件) ，它是一个包装器，需要两个道具，这是不言自明的: i initialValues / i 和 i onSubmit / i。 Formik 包装器跟踪窗体的状态，然后将其加上一些可重用的方法和事件处理程序通过 props 公开给窗体。
...
最后在表单上，我们创建了两个按钮，一个用于取消表单提交，另一个用于提交表单。 取消按钮直接调用 ioncancel / i 回调，而提交按钮触发 Formik 的 onSubmit 事件，该事件又使用组件道具中的 ionsubmit / i 回调。 只有表单 is i valid / i 和 i dirty / i 启用了 Submit 按钮，这意味着用户已经编辑了一些字段。
...
Ionsubmit / i 回调已经从我们的病人列表页面一路传递下来。 它基本上对我们的后端执行一个 HTTP POST，将新返回的病人添加到我们的状态，并关闭模式。 如果后端返回错误，该错误将显示在窗体上。
...
到目前为止，我们已经确定病人可以有不同类型的条目，但是我们还没有任何方法在我们的应用程序中为我们的病人添加条目，所以目前它将是一个非常无用的电子医疗记录。
...
请记住，我们的应用程序中有不同类型的条目，所以我们的后端也应该支持所有这些类型，并检查至少所有必需的字段都为每种类型提供了。
...
如果您愿意，可以为这个练习重用 i Add patient / i 表单中的一些代码，但这不是必需的。
...
注意，文件[ FormField.tsx ]( https://github.com/fullstack-hy2020/patientor/blob/master/src/addpatientmodal/FormField.tsx#l58)有一个红色的组件诊断 / selection，可以用于设置字段 i diagnosis / i:
...
对于范围有限的数值，还有红色制作的组件 NumberField
