...
<!-- The <i>index.js</i> file only imports the actual application from the <i>app.js</i> file and then starts the application. The function _info_ of the logger-module is used for the console printout telling that the application is running. -->
 <i>index.js</i> 文件只从 <i>app.js</i> 文件导入实际的应用，然后启动应用。 logger-module的功能用于控制台打印输出，告诉应用正在运行。
...
在这一章节的练习中，我们将构建一个<i>blog list application</i>，它允许用户保存他们在互联网上偶然发现的有趣博客的信息。 对于每个列出的博客，我们将保存来自用户的作者、标题、 url 和点赞数。
...
让我们从单元测试开始我们的测试之旅。 我们应用的逻辑非常简单，使用单元测试进行测试没有太多意义。 让我们创建一个新的文件<i>utils/for_testing.js</i> ，并编写几个简单的函数，可以用于测试编写实践:
...
**Windows 用户: 如果项目目录的路径包含名称中有空格的目录，** Jest 可能无法工作。 
...
正如我们将在后面看到的，当我们要为一组测试运行一些共享设置或拆卸操作时，<i>describe</i>块是必要的。
...
定义一个新的 _totalLikes_ 函数，该函数接收作为参数的博客文章列表。 该函数返回所有博客文章中<i>likes</i> 的总和。
...
定义一个新的 _favoriteBlog_ 函数，该函数接收一个博客列表作为参数。 这个功能可以找出哪个博客有最多的喜欢。 如果有许多非常喜欢，返回其中一个就足够了。
...
定义一个名为 mostlike 的函数，该函数接收一个 blog 数组作为参数。 该函数返回作者，其博客帖子有最多的赞。 返回值还包含作者收到的赞总数:
