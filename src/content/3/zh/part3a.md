...
在这一部分中，我们的重点转移到后端: 也就是说，转移到在堆栈的服务器端实现功能。
...
我们将在 node.js 的基础上构建我们的后端，这是一个基于 Google 的 Chrome v 8引擎的 JavaScript 运行 https://NodeJS.org/en/  https://developers.Google.com/V8/ 。
...
本课程材料是使用 Node.js 的 i / v10.18.0 / i 版本编写的。 请确保您的 Node 版本至少与材料中使用的版本一样新(您可以通过在命令行中运行 Node-v 来检查版本)。
...
正如在[ part 1](/ en / part1 / JavaScript)中提到的，浏览器还不支持 JavaScript 的最新特性，这就是为什么在浏览器中运行的代码必须是 i / i 加上[ babel ]( https://babeljs.io/ )的原因。 在后端运行 JavaScript 的情况是不同的。 最新版本的 Node 支持大部分最新的 JavaScript 特性，因此我们可以使用最新的特性而不必泄露我们的代码。
...
我们的目标是实现一个后端，它将与[ part 2](/ en / part2 /)中的 notes 应用程序一起工作。 但是，让我们从实现经典的“ hello world”应用程序的基础开始。
...
* * 注意 * * 本部分中的应用程序和练习并不都是 React 应用程序，我们不会使用 i create-React-app / i 实用程序为此应用程序初始化项目。
...
在第2部分中，我们已经提到了[ npm ](/ en / part2 / getting data from server # npm) ，这是一个用于管理 JavaScript 包的工具。 事实上，npm 来源于 Node 生态系统。
...
让我们导航到一个适当的目录，并使用 npminit 命令为应用程序创建一个新模板。 我们将回答该实用程序提出的问题，结果将是在项目根目录下自动生成的 i package.json / i 文件，其中包含有关项目的信息。
...
例如，该文件定义应用程序的入口点是 i index.js / i 文件。
...
接下来，我们创建应用程序的第一个版本，在项目的根目录中添加一个 i index.js / i 文件，代码如下:
...
或者我们可以将它作为一个[ npm 脚本]( https://docs.npmjs.com/misc/scripts 脚本)运行:
...
I start / i npm 脚本之所以有效，是因为我们在 i package.json / i 文件中定义了它:
...
尽管通过从命令行调用 node index.js 来启动项目的执行是有效的，但 npm 项目通常执行 npm 脚本之类的任务。
...
默认情况下，i package.json / i 文件还定义了另一个常用的 npm 脚本，称为 i npm test / i。 由于我们的项目还没有测试库，npm test 命令只是执行以下命令:
...
* * NB * * * 如果端口3001已经被其他应用程序使用，那么启动服务器将导致以下错误消息:
...
你有两个选择。 要么使用端口3001关闭应用程序(材料最后一部分的 json-server 使用端口3001) ，要么为此应用程序使用不同的端口。
...
在第一行中，应用程序导入 Node 的内置[ web 服务器]( https://nodejs.org/docs/latest-v8.x/api/http.html  / 服务器)模块。 这实际上就是我们在浏览器端代码中已经做过的事情，只是语法稍有不同:
...
如今，在浏览器中运行的代码使用 ES6模块。 模块定义为[导出]( https://developer.mozilla.org/en-us/docs/web/javascript/reference/statements/export ) ，并与[导入]( https://developer.mozilla.org/en-us/docs/web/javascript/reference/statements/import )一起使用。
...
然而，Node.js 使用了所谓的[ CommonJS ]( https://en.wikipedia.org/wiki/CommonJS 模块)。 原因在于，早在 JavaScript 在语言规范中支持模块之前，Node 生态系统就对模块有需求。 在撰写本文的时候，Node 还不支持 ES6模块，但是支持它们的 https://nodejs.org/api/esm.html 将会在未来的某个地方出现。
...
该代码使用了[ http ]( https://nodejs.org/docs/latest-v8.x/api/http.html )模块的 createServer 方法来创建一个新的 web 服务器。 一个 i 事件处理程序 / i 被注册到服务器，每次 / i 向服务器的地址发出 HTTP 请求时，它就被调用 i / i  HTTP:/localhost:3001。
...
响应请求的状态代码为200，i Content-Type / i 头文件设置为 i text / plain / i，将返回的站点内容设置为 i Hello World / i。


The last rows bind the http server assigned to the _app_ variable, to listen to HTTP requests sent to the port 3001:
最后一行绑定分配给 app 变量的 HTTP 服务器，监听发送到端口3001的 HTTP 请求:
...
直接使用 Node 内置的[ http ]( https://nodejs.org/docs/latest-v8.x/api/http.html ) web 服务器实现我们的服务器代码是可能的。 但是，它很麻烦，特别是当应用程序增大时。
...
通过提供一个比内置的 http 模块更令人满意的界面，许多库已经开发出来以简化使用 Node 的服务器端开发。 到目前为止，最受欢迎的图书馆是[ http://expressjs.com ]。

Let's take express into use by defining it as a project dependency with the command:
让我们通过下面的命令将它定义为一个项目依赖来使用 express:
...
该依赖项也添加到我们的 i package.json / i 文件中:
...
这些实际上是表达库的依赖项，以及它所有依赖项的依赖项，等等。 这些被称为我们项目的[传递性依赖关系]( https://lexi-lambda.github.io/blog/2016/08/24/understanding-the-npm-dependency-model/ 依赖关系)。
...
4.17.1版本。 在我们的项目中安装了。 在 i package.json / i 中，版本号前面的插入符号是什么意思？
...
Npm 中使用的版本控制模型称为[语义版本控制](semantic versioning) https://docs.npmjs.com/getting-started/semantic-versioning。
...
I ^ 4.17.1 / i 前面的插入符号表示，当项目的依赖项更新时，安装的 express 版本至少为 i 4.17.1 / i。 但是，所安装的 express 版本也可以是具有较大的 i patch / i 号(最后一个数字)或较大的 i minor / i 号(中间的数字)的版本。 第一个 i major / i 号表示的库的主版本必须相同。
...
同样，如果我们在另一台计算机上开始工作，我们可以使用以下命令安装 i package.json / i 中定义的项目的所有最新依赖项:
...
如果依赖项的 i / i 值没有改变，那么新版本应该是[向后兼容的]( https://en.wikipedia.org/wiki/backward_compatibility )。 这意味着，如果我们的应用程序在将来碰巧使用了 express 的版本4.99.175，那么在这个部分中实现的所有代码仍然必须在不对代码进行更改的情况下工作。 相比之下，未来的5.0.0。 版本的快速[可能包含]( https://expressjs.com/en/guide/migrating-5.html )更改，这将导致我们的应用程序不再工作。

### Web and express
网络和快车

Let's get back to our application and make the following changes:
让我们回到我们的应用程序，并进行以下更改:
...
这个应用程序没有太大的改变。 在代码的开头我们导入了 express，这次是一个 i / i 函数，用于创建一个存储在 app 变量中的 express application:
...
接下来，我们定义应用程序的两个 i 路由器 / i。 第一个定义了一个事件处理程序，用于处理对应用程序的 i / / i 根发出的 HTTP GET 请求:
...
事件处理函数接受两个参数。 第一个[ request ]( HTTP://expressjs.com/en/4x/api.html#req )参数包含 HTTP 请求的所有信息，第二个[ response ]( HTTP://expressjs.com/en/4x/api.html#res )参数用于定义请求的响应方式。
...
在我们的代码中，请求是通过使用响应对象的[ send ]( http://expressjs.com/en/4x/api.html#res.send )方法来应答的。 调用该方法使服务器通过发送包含字符串代码 h1 Hello World 的响应响应 HTTP 请求！ / h1 / code，它被传递给 send 方法。 因为参数是一个字符串，所以 express 会自动将 i Content-Type / i 头的值设置为 i text / html / i。 响应的状态代码默认为200。
...
第二个路由定义了一个事件处理程序，它处理对应用程序的 i notes / i 路径发出的 HTTP GET 请求:
...
对于 express，不再需要这样做，因为这种转换是自动发生的。
...
上面的实验是在交互式的[ node-repl ]( https://nodejs.org/docs/latest-v8.x/api/repl.html )中完成的。 您可以通过在命令行中键入 node 来启动交互式 node-repl。 在编写应用程序代码时，对于测试命令的工作方式，repl 特别有用。 我强烈推荐这个！
...
如果我们对应用程序的代码进行更改，我们必须重新启动应用程序以查看更改。 我们通过键入 Ctrl + c 首先关闭应用程序，然后重新启动应用程序来重新启动应用程序。 与 React 中方便的工作流程相比，在 React 中，浏览器在进行更改后自动重新加载，这有点麻烦。
...
I nodemon 将监视启动 nodemon 的目录中的文件，如果任何文件发生更改，nodemon 将自动重启节点应用程序。 我


Let's install nodemon by defining it as a <i>development dependency</i> with the command:
让我们通过下面的命令将 nodemon 定义为 i development dependency / i:
...
如果您不小心使用了错误的命令，并且 nodemon 依赖项被添加到“ dependencies”而不是“ devDependencies” ，那么手动更改 i package.json / i 的内容以匹配上面所示的内容。
...
通过开发依赖性，我们指的是仅在应用程序开发过程中需要的工具，例如用于测试或自动重新启动应用程序的工具，如 i nodemon / i。
...
当应用程序在生产服务器(例如 Heroku)的生产模式下运行时，不需要这些开发依赖项。


We can start our application with <i>nodemon</i> like this:
我们可以用 i nodemon / i 这样开始我们的应用程序:
...
对应用程序代码的更改现在会导致服务器自动重新启动。 值得注意的是，即使后端服务器自动重启，浏览器仍然需要手动刷新。 这是因为不像在 React 中工作，我们甚至不具备自动重新加载浏览器所需的[热重新加载](hot reload) https://gaearon.github.io/React-hot-loader/getstarted/ 。


The command is long and quite unpleasant, so let's define a dedicated <i>npm script</i> for it in the <i>package.json</i> file:
这个命令很长，而且相当令人不快，所以让我们在 i package.json / i 文件中为它定义一个专用的 i npm 脚本 / i:
...
在脚本中，不需要指定 i 节点 modules / 。 Bin / nodemon / i 到 nodemon 的路径，因为 npm 自动知道从该目录搜索文件。


We can now start the server in the development mode with the command:
我们现在可以在开发模式下使用以下命令启动服务器:
...
让我们扩展我们的应用程序，以便它提供 RESTful HTTP API 作为[ json-server ]( https://github.com/typicode/json-server#routes 服务器)。
...
REST，又名。 Rest 是在2000年 Roy Fielding 的[论文]( https://www.ics.uci.edu/~Fielding/pubs/dissertation/rest_arch_style.htm )中引入的。 Rest 是一种架构风格，用于构建可伸缩的 web 应用程序。
...
我们不会深入探究 Fielding 对 REST 的定义，也不会花时间思考什么是 RESTful，什么不是 RESTful。 相反，我们只关注网络应用程序对 RESTful API 的典型理解，从而采取了一种更为狭隘的 https://en.wikipedia.org/wiki/representational_state_transfer#applied_to_web_services。 Rest 的最初定义实际上并不局限于 web 应用程序。
...
我们在[上一部分](/ en / part2 / modified data in server # rest)中提到过，在我们的应用程序中，像注释这样的奇异事物，在 RESTful thinking 中称为 i resources / i。 每个资源都有一个相关联的 URL，这个 URL 是资源的唯一地址。
...
一个约定是通过结合资源类型的名称和资源的唯一标识符来为资源创建唯一的地址。
...
我们可以对资源执行不同的操作。要执行的操作由 HTTP i verb / i 定义:
...
| notes / 10 | DELETE | 删除标识的资源 | 
| notes/10              | PUT                 | replaces the entire identified resource with the request data
| notes / 10 | PUT | 用请求数据替换整个标识的资源
| notes/10              | PATCH               | replaces a part of the identified resource with the request data
| notes / 10 | PATCH | 用请求数据替换已识别资源的一部分
|                       |                     |                                                                  |
|                       |                     |                                                                  |
...
这就是我们如何设法粗略地定义 REST 所指的[统一接口]( https://en.wikipedia.org/wiki/representational_state_transfer#architectural_constraints 接口) ，这意味着一种一致的定义接口的方式，使系统能够进行合作。
...
这种解释 REST 的方式在 Richardson Maturity Model 属于[ RESTful 成熟度的第二个层次]( https://martinfowler.com/articles/richardsonmaturitymodel.html )。 根据 Roy Fielding 提供的定义，我们实际上并没有定义一个[ REST API ]( http://Roy.gbiv.com/untangled/2008/REST-apis-must-be-hypertext-driven )。 事实上，大多数世界上所谓的“ REST” API 不符合 Fielding 在其论文中概述的原始标准。
...
在某些地方(例如[ Richardson，Ruby: RESTful Web Services ]( http://shop.oreilly.com/product/9780596529260.do )) ，你会看到我们为一个简单的[ CRUD ]( https://en.wikipedia.org/wiki/create,_read,_update_and_delete ) API 建立的模型，被称为[面向资源架构]( https://en.wikipedia.org/wiki/resource-oriented_architecture )的一个例子，而不是 REST。 我们将避免陷入语义学的争论，而是回到应用程序的工作中。
...
让我们扩展我们的应用程序，以便它提供一个 REST 接口，用于操作单个注释。 首先，让我们创建一个[ route ]( http://expressjs.com/en/guide/routing.html )来获取单个资源。
...
我们将为单个纸币使用的唯一地址是 i notes / 10 / i，其中末尾的数字指的是纸币的唯一 id 号。
...
我们可以使用冒号语法为快递路由定义[ parameters ]( http://expressjs.com/en/guide/routing.html#route-parameters ) :
...
现在，代码 app.GET (’ / api / notes / : id’ ，...) / 代码将处理所有的 HTTP GET 请求，这些请求的格式是 i / api / notes / SOMETHING / i，其中 i SOMETHING / i 是任意的字符串。
...
当我们通过在浏览器中进入 http://localhost:3001/api/notes/1来测试我们的应用程序时，我们注意到它似乎不工作，因为浏览器显示一个空白页面。 这对于我们软件开发人员来说并不奇怪，现在是调试的时候了。
...
来自 route 的 id 参数被传递给我们的应用程序，但是 find 方法没有找到匹配的注释。
...
为了进一步研究，我们还在传递给 find 方法的比较函数中添加了控制台日志。 为了做到这一点，我们必须去掉紧凑箭头函数语法 em note note.id / em，并使用语法和一个显式的 return 语句:
...
返回的 HTTP状态码是200，这意味着响应成功了。 由于 i content-length / i 标头的值为0，因此不会将数据与响应一起发送回来，并且可以从浏览器验证这一点。
...
由于响应没有附加任何数据，我们使用[ status ]( http://expressjs.com/en/4x/api.html#res.status )方法来设置状态，并使用[ end ]( http://expressjs.com/en/4x/api.html#res.end )方法来响应请求而不发送任何数据。
...
If-condition 利用了这样一个事实，即所有的 JavaScript 对象都是[ truthy ]( https://developer.mozilla.org/en-us/docs/glossary/truthy ) ，这意味着它们在比较操作中被评估为 true。 然而，undefined is [ falsy ]( https://developer.mozilla.org/en-us/docs/glossary/falsy )意思是它将评估为 false。
...
我们的应用程序正常工作，如果没有发现注释，则发送错误状态代码。 然而，应用程序不会返回任何东西显示给用户，就像 web 应用程序通常在我们访问一个不存在的页面时所做的那样。 我们实际上不需要在浏览器中显示任何内容，因为 REST API 是用于编程使用的接口，只需要错误状态代码。
...
如果资源不存在，对于应该向 DELETE 请求返回什么状态代码没有共识。 实际上，只有204和404两个选项。 为了简单起见，我们的应用程序在这两种情况下都将响应204。
...
有许多工具可以使后端的测试更加容易。 其中之一就是命令行程序[ curl ]( https://curl.haxx.se 文件) ，这个命令行程序在本文前面的部分中已经简要地提到过。

Instead of curl, we will take a look at using [Postman](https://www.getpostman.com/) for testing the application.
代替 curl，我们将使用[ Postman ](Postman) https://www.getpostman.com/ 来测试应用程序。
...
因为应用程序中的注释只保存到内存中，所以当我们重新启动应用程序时，注释列表将返回到原始状态。
...
如果你使用 Visual Studio Code，你可以使用 VS Code [ REST 客户端](休息 /  https://marketplace.visualstudio.com/items?itemname=humao.REST-client )插件代替邮递员。
...
一旦插件安装完毕，使用起来非常简单。 我们在应用程序的根目录名为 i requests / i。 我们将目录中的所有 REST 客户机请求保存为以 i 结尾的文件。 休息 / 放松。
...
事件处理函数可以从请求对象的 i body / i 属性访问数据。
...
如果没有 json-parser，i body / i 属性将是未定义的。 Json-parser 的功能是获取请求的 JSON 数据，将其转换为 JavaScript 对象，然后在调用路由处理程序之前将其附加到请求对象的 i body / i 属性。


For the time being, the application does not do anything with the received data besides printing it to the console and sending it back in the response.
目前，除了将接收到的数据打印到控制台并在响应中将其发送回来之外，应用程序不对其执行任何操作。
...
类似地，检查控制台以确保后端在不同情况下的行为与我们期望的一样，比如在使用 HTTP POST 请求发送数据时。 当然，在开发应用程序时向代码中添加大量 em console.log / em 命令是一个好主意。
...
如果头部没有正确的值，服务器将无法正确解析数据。 它甚至不会去猜测数据的格式，因为有大量的潜在 i Content-Types / i  https://developer.mozilla.org/en-us/docs/web/http/basics_of_http/mime_types。
...
我们为这个请求创建了一个新的 i create  note.rest / i 文件，这个请求是根据[文档中的说明]( https://github.com/huachao/vscode-restclient/blob/master/readme.md#usage )格式化的。
...
Rest 客户机相对于 Postman 的一个好处是，请求可以在项目存储库的根部轻松获得，并且可以分发给开发团队中的每个人。 邮递员也允许用户保存请求，但是当你在处理多个不相关的项目时，情况会变得非常混乱。
...
有时在进行调试时，您可能希望了解 HTTP 请求中设置了哪些头。 实现这一点的一种方法是通过请求对象的[ get ]( http://expressjs.com/en/4x/api.html#req.get )方法，该方法可用于获取单个头的值。 Request 对象还具有 i headers / i 属性，该属性包含特定请求的所有头部。
>
>

> Problems can occur with the VS REST client if you accidentally add an empty line between the top row and the row specifying the HTTP headers. In this situation, the REST client interprets this to mean that all headers are left empty, which leads to the backend server not knowing that the data it has received is in the JSON format.
如果您不小心在指定 HTTP 标头的顶行和行之间添加了一个空行，那么 VS REST 客户机可能会出现问题。 在这种情况下，REST 客户机将其解释为所有头都是空的，这导致后端服务器不知道它接收的数据是 JSON 格式的。
>
>
...
如果您在代码中的某个位置使用 console.log (request.headers)命令打印所有请求标头，那么您将能够发现缺少 i Content-Type / i 标头。
...
让我们回到应用程序。 一旦我们知道应用程序正确地接收数据，就是时候最终处理请求了:
...
我们需要一个唯一的 id。 首先，找出当前列表中最大的 id 号，并将其分配给 maxId 变量。 然后将新通知的 id 定义为 maxId + 1。 这种方法实际上是不被推荐的，但是我们将暂时接受它，因为我们很快就会取代它。
...
当前版本仍然存在 HTTP POST 请求可用于添加具有任意属性的对象的问题。 让我们通过定义 i content / i 属性不能为空来改进应用程序。 I important / i 和 i date / i 属性将被赋予默认值。 所有其他属性都被丢弃:
...
请注意，调用 return 是至关重要的，因为否则代码将执行到最后，并将格式不正确的通知保存到应用程序中。
...
如果 content 属性具有值，则说明将基于接收到的数据。 正如前面提到的，在服务器上生成时间戳比在浏览器上生成更好，因为我们不能相信运行浏览器的主机的时钟设置是正确的。 现在由服务器生成 i date / i 属性。
...
如果保存在 body 变量中的数据具有 i important / i 属性，则表达式将计算为其值。 如果该属性不存在，那么表达式将计算为 false，该表达式在垂直线的右侧定义。
...
您可以在[ this github repository ]的 i part3-1 / i 分支中找到我们当前应用程序的全部代码 https://github.com/fullstack-hy2020/part3-notes-backend/tree/part3-1。
...
如果您克隆了项目，在启动应用程序之前运行 npm install 命令，使用 npm start 或 npm run dev。
...
注意: 建议将本部分的所有练习放到一个新的专用 git 存储库中，并将源代码放在存储库的根部。 否则你会在练习3.10中遇到问题。
...
* * NB: * * 因为这不是一个前端项目，我们没有使用 React，所以强大的应用程序没有创建 / 强大的 create-React-app。 您可以使用 em npm init / em 命令初始化这个项目，该命令在本部分的前面已经演示过了。


**Strong recommendation:** When you are working on backend code, always keep an eye on what's going on in the terminal that is running your application.
* * 强烈建议: * * 当你在处理后端代码时，始终关注运行应用程序的终端中发生了什么。
...
请注意，route i api / persons / i 中的正斜杠不是特殊字符，它与字符串中的任何其他字符一样。


The application must be started with the command _npm start_.
应用程序必须以命令 npm start 启动。
...
应用程序还必须提供 npm run dev 命令，该命令将运行应用程序，并在进行更改并将更改保存到源代码中的文件时重新启动服务器。
...
I 特别是，已经建立了一个约定，即 GET 和 HEAD 方法除了检索之外不应该有采取其他行动的意义。 这些方法应该被认为是“安全的”。 我
...
没有什么能够保证 GET 请求实际上是 i safe / i，这实际上只是 HTTP 标准中定义的一个建议。 通过遵守我们的 API 中的 RESTful 原则，GET 请求实际上总是以一种 i safe / i 的方式使用。
...
I 方法还可以具有“幂等”属性，即(除了错误或过期问题) n0相同请求的副作用与单个请求相同。 方法 GET、 HEAD、 PUT 和 DELETE 共享此属性 / i
...
如果我们对 url i / api / notes / 10 / i 发出 HTTP PUT 请求，并且在发出请求时发送数据 em { content: “没有副作用! ” ，重要: true } / em，结果是相同的，不管多少次请求被发送。


Like <i>safety</i> for the GET request, <i>idempotence</i> is also just a recommendation in the HTTP standard and not something that can be guaranteed simply based on the request type. However, when our API adheres to RESTful principles, then GET, HEAD, PUT, and DELETE requests are used in such a way that they are idempotent.
就像 GET 请求的 i safety / i 一样，i 幂等 / i 也只是 HTTP 标准中的一个推荐，而不是仅仅基于请求类型就可以保证的东西。 但是，当我们的 API 遵循 RESTful 原则时，GET、 HEAD、 PUT 和 DELETE 请求的使用方式是等幂的。
...
在实践中，您可以同时使用多个中间件。 当你有多于一个的时候，他们被一个接一个地执行，按照他们被使用的顺序。


Let's implement our own middleware that prints information about every request that is sent to the server.
让我们实现我们自己的中间件，它打印有关发送到服务器的每个请求的信息。
...
在函数体的末尾，调用作为参数传递的下一个函数。 下一个函数将控制权交给下一个中间件。
...
中间件函数按照与表达服务器对象的使用方法一起使用的顺序调用。 请注意，json-parser 是在 requestLogger 中间件之前使用的，因为否则在执行日志记录器时，不会初始化我的 request.body / i！
...
让我们在路由之后添加以下中间件，它用于捕获对不存在的路由发出的请求。 对于这些请求，中间件将返回 JSON 格式的错误消息。
...
在你的日志应用程序中添加[ morgan ]( https://github.com/expressjs/morgan 中间件)。 将其配置为基于 i tiny / i 配置将消息记录到控制台。

The documentation for Morgan is not the best, and you may have to spend some time figuring out how to configure it correctly. However, most documentation in the world falls under the same category, so it's good to learn to decipher and interpret cryptic documentation in any case.
Morgan 的文档不是最好的，您可能需要花费一些时间来弄清楚如何正确地配置它。 然而，世界上大多数文档都属于同一类别，因此无论如何，学习解释和解释神秘的文档都是有益的。
...
这个练习可以通过几种不同的方式来完成。其中一种可能的解决方案利用了以下两种技巧:
