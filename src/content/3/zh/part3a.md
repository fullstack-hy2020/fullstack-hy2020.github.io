...
我们的目标是实现一个后端，它将与[ part 2](/ en / part2 /)中的 notes 应用一起工作。 但是，让我们从实现经典的“ hello world”应用的基础开始。
...
* * 注意 * * 本部分中的应用和练习并不都是 React 应用，我们不会使用<i>create-React-app</i> 实用程序为此应用初始化项目。
...
让我们导航到一个适当的目录，并使用 npminit 命令为应用创建一个新模板。 我们将回答该实用程序提出的问题，结果将是在项目根目录下自动生成的<i>package.json</i> 文件，其中包含有关项目的信息。
...
* * NB * * * 如果端口3001已经被其他应用使用，那么启动服务器将导致如下错误消息:
...
你有两个选择。 要么使用端口3001关闭应用(材料最后一部分的 json-server 使用端口3001) ，要么为此应用使用不同的端口。
...
在第一行中，应用导入 Node 的内置[ web 服务器]( https://nodejs.org/docs/latest-v8.x/api/http.html  / 服务器)模块。 这实际上就是我们在浏览器端代码中已经做过的事情，只是语法稍有不同:
...
直接使用 Node 内置的[ http ]( https://nodejs.org/docs/latest-v8.x/api/http.html ) web 服务器实现我们的服务器代码是可能的。 但是，它很麻烦，特别是当应用增大时。
...
如果依赖项的 i / i 值没有改变，那么新版本应该是[向后兼容的]( https://en.wikipedia.org/wiki/backward_compatibility )。 这意味着，如果我们的应用在将来碰巧使用了 express 的版本4.99.175，那么在这个部分中实现的所有代码仍然必须在不对代码进行更改的情况下工作。 相比之下，未来的5.0.0。 版本的快速[可能包含]( https://expressjs.com/en/guide/migrating-5.html )更改，这将导致我们的应用不再工作。
...
这个应用没有太大的改变。 在代码的开头我们导入了 express，这次是一个 i / i 函数，用于创建一个存储在 app 变量中的 express application:
...
接下来，我们定义应用的两个<i>路由器</i>。 第一个定义了一个事件处理程序，用于处理对应用的<i>/</i> 根发出的 HTTP GET 请求:
...
第二个路由定义了一个事件处理程序，它处理对应用的<i>notes</i> 路径发出的 HTTP GET 请求:
...
上面的实验是在交互式的[ node-repl ]( https://nodejs.org/docs/latest-v8.x/api/repl.html )中完成的。 您可以通过在命令行中键入 node 来启动交互式 node-repl。 在编写应用代码时，对于测试命令的工作方式，repl 特别有用。 我强烈推荐这个！
...
如果我们对应用的代码进行更改，我们必须重新启动应用以查看更改。 我们通过键入 Ctrl + c 首先关闭应用，然后重新启动应用来重新启动应用。 与 React 中方便的工作流程相比，在 React 中，浏览器在进行更改后自动重新加载，这有点麻烦。
...
I nodemon 将监视启动 nodemon 的目录中的文件，如果任何文件发生更改，nodemon 将自动重启节点应用。 我
...
通过开发依赖性，我们指的是仅在应用开发过程中需要的工具，例如用于测试或自动重新启动应用的工具，如<i>nodemon</i>。
...
当应用在生产服务器(例如 Heroku)的生产模式下运行时，不需要这些开发依赖项。


We can start our application with <i>nodemon</i> like this:
我们可以用<i>nodemon</i> 这样开始我们的应用:
...
对应用代码的更改现在会导致服务器自动重新启动。 值得注意的是，即使后端服务器自动重启，浏览器仍然需要手动刷新。 这是因为不像在 React 中工作，我们甚至不具备自动重新加载浏览器所需的[热重新加载](hot reload) https://gaearon.github.io/React-hot-loader/getstarted/ 。
...
让我们扩展我们的应用，以便它提供 RESTful HTTP API 作为[ json-server ]( https://github.com/typicode/json-server#routes 服务器)。
...
REST，又名。 Rest 是在2000年 Roy Fielding 的[论文]( https://www.ics.uci.edu/~Fielding/pubs/dissertation/rest_arch_style.htm )中引入的。 Rest 是一种架构风格，用于构建可伸缩的 web 应用。
...
我们不会深入探究 Fielding 对 REST 的定义，也不会花时间思考什么是 RESTful，什么不是 RESTful。 相反，我们只关注网络应用对 RESTful API 的典型理解，从而采取了一种更为狭隘的 https://en.wikipedia.org/wiki/representational_state_transfer#applied_to_web_services。 Rest 的最初定义实际上并不局限于 web 应用。
...
在某些地方(例如[ Richardson，Ruby: RESTful Web Services ]( http://shop.oreilly.com/product/9780596529260.do )) ，你会看到我们为一个简单的[ CRUD ]( https://en.wikipedia.org/wiki/create,_read,_update_and_delete ) API 建立的模型，被称为[面向资源架构]( https://en.wikipedia.org/wiki/resource-oriented_architecture )的一个例子，而不是 REST。 我们将避免陷入语义学的争论，而是回到应用的工作中。
...
当我们通过在浏览器中进入 http://localhost:3001/api/notes/1来测试我们的应用时，我们注意到它似乎不工作，因为浏览器显示一个空白页面。 这对于我们软件开发人员来说并不奇怪，现在是调试的时候了。
...
当我们在浏览器中再次访问 http://localhost:3001/api/notes/1时，这个终端控制台将显示如下内容:
...
如果资源不存在，对于应该向 DELETE 请求返回什么状态代码没有共识。 实际上，只有204和404两个选项。 为了简单起见，我们的应用在这两种情况下都将响应204。
...
代替 curl，我们将使用[ Postman ](Postman) https://www.getpostman.com/ 来测试应用。
...
目前，除了将接收到的数据打印到控制台并在响应中将其发送回来之外，应用不对其执行任何操作。
...
在实现应用逻辑的其余部分之前，让我们先与 Postman 验证服务器实际接收到的数据。 除了在 Postman 中定义 URL 和请求类型外，我们还必须定义在<i>body</i> 中发送的数据:
...
* * NB * * * 当你在后端工作时，让运行应用的终端始终可见 / i。 多亏了 Nodemon，我们对代码所做的任何更改都将重新启动应用。 如果你注意控制台，你会立即发现应用中出现的错误:
...
类似地，检查控制台以确保后端在不同情况下的行为与我们期望的一样，比如在使用 HTTP POST 请求发送数据时。 当然，在开发应用时向代码中添加大量 em console.log / em 命令是一个好主意。
...
让我们回到应用。 一旦我们知道应用正确地接收数据，就是时候最终处理请求了:
...
当前版本仍然存在 HTTP POST 请求可用于添加具有任意属性的对象的问题。 让我们通过定义<i>content</i> 属性不能为空来改进应用。<i>important</i> 和<i>date</i> 属性将被赋予默认值。 所有其他属性都被丢弃:
...
请注意，调用 return 是至关重要的，因为否则代码将执行到最后，并将格式不正确的通知保存到应用中。
...
您可以在[ this github repository ]的<i>part3-1</i> 分支中找到我们当前应用的全部代码 https://github.com/fullstack-hy2020/part3-notes-backend/tree/part3-1。
...
注意，存储库的主分支包含应用的后一个版本的代码。 应用当前状态的代码特别在 branch [ part3-1]( https://github.com/fullstack-hy2020/part3-notes-backend/tree/part3-1)中。
...
如果您克隆了项目，在启动应用之前运行 npm install 命令，使用 npm start 或 npm run dev。
...
* * NB: * * 因为这不是一个前端项目，我们没有使用 React，所以强大的应用没有创建 / 强大的 create-React-app。 您可以使用 em npm init / em 命令初始化这个项目，该命令在本部分的前面已经演示过了。
...
您可以在[ this github repository ]的<i>part3-2</i> 分支中找到我们当前应用的全部代码，该分支位于 https://github.com/fullstack-hy2020/part3-notes-backend/tree/part3-2文件库中。
...
在你的日志应用中添加[ morgan ]( https://github.com/expressjs/morgan 中间件)。 将其配置为基于<i>tiny</i> 配置将消息记录到控制台。
