...
在我们的上下文中，问题在于，默认情况下，运行在浏览器中的应用程序的 JavaScript 代码只能与同一[原点]的服务器通信( https://developer.mozilla.org/en-us/docs/web/security/same-origin_policy )。
...
请记住，[同源策略]( https://developer.mozilla.org/en-us/docs/web/security/same-origin_policy )和 CORS 并不是特定于 React 或 Node 的。 它们实际上是 web 应用程序操作的通用原则。
...
现在整个堆栈已经准备就绪，让我们将应用程序移动到互联网上。 我们将使用古老的 Heroku  https://www.Heroku.com。
...
向项目的根目录添加一个名为 i Procfile / i 的文件，告诉 Heroku 如何启动应用程序。
...
更改应用程序在 i index.js / i 文件底部使用的端口的定义，如下所示:
...
使用命令 i Heroku Create / i 创建一个 Heroku 应用程序，在 application 目录中创建一个 Git 仓库，提交代码并将其移动到 Heroku，命令 i Git push Heroku master / i。
...
到目前为止，我们一直在 i 开发模式 / i 中运行 React code。 在开发模式下，应用程序被配置为提供清晰的错误消息，立即向浏览器呈现代码更改，等等。
...
当应用程序被部署时，我们必须创建一个[生产构建]( https://reactjs.org/docs/optimizing-performance.html#use-the-production-build )或一个为生产而优化的应用程序版本。
...
这将创建一个名为 i build / i 的目录(其中包含应用程序中唯一的 HTML 文件 i index. HTML / i) ，其中包含目录 i static / i。 我们应用程序的 JavaScript 代码的[ Minified ]( https://en.wikipedia.org/wiki/minification_(programming))版本将生成到 i static / i 目录。 即使应用程序代码位于多个文件中，所有的 JavaScript 都将被缩小到一个文件中。 实际上，来自所有应用程序依赖项的所有代码也将缩小到这个单一文件中。
...
部署前端的一个选择是将生产构建(i build / i 目录)复制到后端存储库的根目录，并配置后端以显示前端的 i main page / i (文件 i build / index.html / i)作为其主页。
...
该文件包含一些指令，用于获取定义应用程序样式的 CSS 样式表，以及两个 i script / i 标记，这些标记指示浏览器获取应用程序的 JavaScript 代码——实际的 React 应用程序。
...
如果这个项目是用 create-react-app 创建的，那么这个问题很容易解决。 将以下声明添加到前端存储库的 i package.json / i 文件中就足够了。
...
在重新启动之后，React 开发环境将作为一个[代理]( https://create-React-app.dev/docs/proxying-api-requests-in-development/ )工作。 如果 React 代码对服务器地址发出了一个 HTTP 请求，该地址位于 i  HTTP://localhost:3000 / i，而不是 React 应用程序本身管理的地址(即当请求不是为了获取应用程序的 CSS 或 JavaScript) ，那么该请求将被重定向到 i  HTTP://localhost:3001 / i 的服务器。
...
在某些情况下，将前端代码部署为它自己的应用程序可能是明智的。 通过创建-反应-应用程序创建的应用程序是[简单的]( https://github.com/mars/create-react-app-buildpack )。
...
专业提示: 当你将应用程序部署到 Heroku 时，至少在开始的时候使用命令 em herlogs-t / em 关注 Heroku 应用程序的日志是值得的。
...
生成前端的生产构建，并使用本部分介绍的方法将其添加到 internet 应用程序中。
