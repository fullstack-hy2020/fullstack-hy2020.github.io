...
在[老式 web 应用](/ en / part0 / web 应用基础 # traditional-web-applications)中，更改应用显示的页面将由浏览器向服务器发出 HTTP GET 请求并显示表示返回视图的 HTML 来完成。
...
路由，或基于浏览器中的 url /<i>的组件的条件渲染，通过将组件放置为 i Router</i> 组件的子组件来使用，这意味着在<i>Router</i>-tags 内部。
...
如果您将该项目克隆到现有的 git 存储库中，请记住我删除克隆应用的 git 配置: / i
