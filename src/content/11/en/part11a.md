...
If a developer is working correctly in their own environment, it is unlikely that they will push code that would fail linting or tests. They would have already checked that on their own machine. The build too would likely pass because the tests would have likely had something to say about any issues that would prevent a build. It is possible that there will be a failure related to code in the master branch from other developer's changes and finding these issues is arguably the most important job of CI.

But what about packaging and deploying then? Packaging is often an area where issues crop up in CI as this isn't something that's usually tested locally. It makes sense to test the packaging of a project during the CI workflow even if we don't do anything with the resultant package.
