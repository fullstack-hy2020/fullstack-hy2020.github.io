...
**Note:** If you use [react-native-picker](https://www.npmjs.com/package/react-native-picker-select) there is a chance that you can get an error like `"RNCAndroidDialogPicker" was not found in the UIManager`. In such case, you can set the version to `"react-native-picker-select": "^7.0.0"` in your `package.json`. Then delete the `node_modules` folder and install again the dependencies with `npm install`. More info [here](https://github.com/react-native-picker/picker/issues/136#issuecomment-693013536). Alternatively, you can use [react-native-picker](https://github.com/react-native-picker/picker), by adding `"@react-native-community/picker": "1.6.0"` and then install it with `npm install`.
