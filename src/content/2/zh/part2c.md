...
一段时间以来，我们只致力于“前端” ，即客户端(浏览器)功能。 我们将在本课程的第三章节开始研究“后端” ，即服务器端功能。 尽管如此，我们现在将向这个方向迈出一步，熟悉在浏览器中执行的代码如何与后端通信。
...
接下来，我们的想法是将笔记保存到服务器，这在本例中意味着将笔记保存到 json-server。 React代码从服务器获取笔记并将其渲染到屏幕上。 无论何时向应用添加新注释，React 代码都会将其发送到服务器，以使新注释保存在“内存”中。
...
在本课程的[第3章节](/zh/part3)中，我们将更详细地了解实现服务器端功能的原则。
...
我们的第一个任务是从地址 /  http://localhost:3001/notes 获取已经存在的笔记到 React 应用。
...
为了让浏览器保持<i>responsive</i>，即能够以足够的速度连续地对用户操作作出React，代码逻辑需要使任何单一的计算都不会花费太长的时间。
...
我们将在[课程的第三章节](/ en / part3)中更加熟悉 npm 工具。
...
参数之间有细微的差别。<i>axios</i> 被安装为应用的运行时依赖项(-- save) ，因为程序的执行需要库的存在。 另一方面，i json-server / i 是作为开发依赖项(-- save-dev)安装的，因为程序本身并不需要它。 它用于在软件开发过程中提供帮助。 在课程的下一部分将会有更多关于不同依赖关系的内容。
...
让我们首先通过将表示应用的<i>App</i> 组件放在回调函数中来“糟糕地”完成这个任务。 这是通过将<i>index.js</i> 更改为如下形式来实现的:
...
让我们从<i>index.js</i> 中删除数据的获取。 不再需要将数据作为props传递给<i>App</i> 组件。 所以我将 index.js / i 简化为:
...
首先执行定义组件的函数体，并首次渲染组件。 这时我打印了 render 0 notes / i，这意味着还没有从服务器获取数据。
...
在渲染完成后立即执行。 函数的执行结果是<i>effect</i> 被打印到控制台，em axios.get / em 命令启动从服务器获取数据的操作，并将如下函数注册为操作的<i>事件处理程序</i>:
...
通常，对状态更新函数的调用会触发组件的重新渲染。 结果，i render 3 notes / i 被打印到控制台，从服务器获取的注释被显示到屏幕上。
...
因此，默认情况下，效果是<i>总是</i> 在组件渲染之后运行。 然而，在我们的例子中，我们只想在第一次渲染的时候执行这个效果。
...
Em / useEffect / em 的第二个参数用于[指定效果运行的频率]( https://reactjs.org/docs/hooks-reference.html#conditionally-firing-an-effect )。 如果第二个参数是一个空数组 em [] / em，那么这个效果只在组件的第一次渲染时运行。
...
我们的应用仍然有一个问题。当添加新的笔记时，它们不存储在服务器上。
...
在开发的这个阶段，应用的所有部分都驻留在软件开发人员的机器上，也就是本地主机。 当应用被部署到互联网上时，情况发生了变化。 我们将在第三章节做这个。
...
我们继续开发电话簿。 将应用的初始状态存储在文件<i>db.json</i> 中，该文件应该放在项目的根目录中。
...
修改应用，使用<i>axios</i>-library 从服务器获取数据的初始状态。 使用[ Effect hook ]( https://reactjs.org/docs/hooks-Effect.html )完成获取操作。
...
创建一个应用，其中可以查看不同国家的数据。 应用可能应该从端点[ all ]( https://restcountries.eu/#api-endpoints-all )获取数据。
...
* * 注意: 你的应用在大多数国家都能工作就足够了。 有些国家，如苏丹 / 苏丹 / 苏丹，可能会制造麻烦，因为国名是另一个国家名称的一部分，即南苏丹 / 苏丹 / 苏丹。 你不必担心这些边缘情况。

**WARNING** create-react-app will automatically turn your project into a git-repository unless you create your application inside of an existing git repository. **Most likely you do not want each of your projects to be a separate repository**, so simply run the _rm -rf .git_ command at the root of your application.
* * 警告 * * create-react-app 将自动将您的项目转换为 git-repository，除非您在现有 git 存储库中创建应用。 * * 很可能您不希望每个项目都是单独的存储库，因此只需运行 rm-rf。 在应用的根目录下使用 git 命令。
...
在这个练习中，您的应用能够在大多数国家工作就足够了。 以其他国家的名义出现的国家，如<i>苏丹</i> 可以被忽略。
