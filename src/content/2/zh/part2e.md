...
我们当前应用程序的外观是相当适度的。 在[练习0.2](/ en / part0 / web apps 基础 # exercises-0-1-0-6)中，作业是浏览 Mozilla 的[ CSS 教程]( https://developer.Mozilla.org/en-us/docs/learn/getting_started_with_the_web/css_basics )。
...
在进入下一部分之前，让我们看看如何向 React 应用程序添加样式。 有几种不同的方法可以做到这一点，我们将在稍后介绍其他的方法。 首先，我们将以传统的方式将 CSS 添加到我们的应用程序中; 在一个单独的文件中，不使用[ CSS 预处理器]( https://developer.mozilla.org/en-us/docs/glossary/css_preprocessor )(尽管这并不完全正确，我们将在后面学习)。
...
让我们在 i src / i 目录下添加一个新的 i index.css / i 文件，然后通过导入 i index.js / i 文件将其添加到应用程序中:
...
让我们在 i index.CSS / i 文件中添加以下 CSS 规则:
...
Css 规则由 i 选择器 / i 和 i 声明 / i 组成。 选择器定义规则应该应用于哪些元素。 上面的选择器是 i h1 / i，它将匹配我们应用程序中的所有 i h1 / i 头标记。
...
一个 CSS 规则可以包含任意数量的属性。 让我们修改前面的规则，通过将字体样式定义为 i italic / i:
...
使用不同类型的 CSS 选择器有许多匹配元素的方法，比如 https://developer.mozilla.org/en-us/docs/web/CSS/css_selectors。
...
如果我们想针对每个音符的风格，我们可以使用选择器 i li / i，因为所有音符都包装在 i li / i 标签中:
...
让我们在样式表中加入以下规则(因为我对优雅网页设计的知识接近于零，所以样式没有多大意义) :
...
使用元素类型来定义 CSS 规则有点问题。 如果我们的应用程序包含其他 i li / i 标记，那么同样的样式规则也应用于它们。
...
如果我们想把我们的风格特别地应用到笔记上，那么最好使用[类选择器]( https://developer.mozilla.org/en-us/docs/web/css/class_selectors )。
...
在反应中，我们必须使用[ className ]( https://reactjs.org/docs/dom-elements.html#className )属性而不是 class 属性。 考虑到这一点，让我们对 i Note / i 组件进行以下更改:
...
如果您现在向应用程序添加其他 i li / i 元素，它们将不会受到上述样式规则的影响。
...
我们先前实现了当用户试图通过 em alert / em 方法切换删除注释的重要性时显示的错误消息。 让我们将错误消息实现为它自己的 React 组件。
...
如果 em message / em prop 的值为 em null / em，则不会向屏幕呈现任何内容，在其他情况下，消息会在 div 元素中呈现。
...
现在，我们准备添加显示错误消息的逻辑。 让我们用下面的方法更改 em toggleimportance / em 函数:
...
当出现错误时，我们向 em errorMessage / em 状态添加一个描述性错误消息。 与此同时，我们启动一个计时器，它将在5秒后将 em errorMessage / em 状态设置为 em null / em。
...
我们应用程序当前状态的代码可以在[ github ]( https://github.com/fullstack-hy2020/part2-notes/tree/part2-7)上的 i part2-7 / i 分支中找到。
...
反应也使得直接在代码中编写样式成为可能，即所谓的[内联样式](inline styles) https://React-cn.github.io/React/tips/inline-styles.html。
...
Css 规则在 JavaScript 中的定义与普通 CSS 文件中的定义稍有不同。 假设我们想给一些元素颜色绿色和斜体字体，大小为16像素。 在 CSS 中，它看起来像这样:
...
每个 CSS 属性都被定义为 JavaScript 对象的一个独立属性。 像素的数值可以简单地定义为整数。 与常规 CSS 相比，一个主要的区别是连字符(烤串大小写)的 CSS 属性是用 camelCase 编写的。
...
接下来，我们可以通过创建一个 i Footer / i 组件向应用程序添加一个“ bottom block” ，并为它定义以下行内样式:
...
内联样式有一定的限制，例如，所谓的[伪类]( https://developer.mozilla.org/en-us/docs/web/css/pseudo-classes 类)不能直接使用。
...
内联样式和其他一些将样式添加到 React 组件的方法完全违背了旧的惯例。 传统上，将 CSS 与内容(HTML)和功能(JavaScript)完全分离被认为是最佳实践。 根据这个古老的思想流派，我们的目标是将 CSS、 HTML 和 JavaScript 编写到它们各自的文件中。


The philosophy of React is, in fact, the polar opposite of this. Since the separation of CSS, HTML, and JavaScript into separate files did not seem to scale well in larger applications, React bases the division of the application along the lines of its logical functional entities.
反应的哲学，事实上，是这个极端的对立面。 由于将 CSS、 HTML 和 JavaScript 分离成单独的文件在大型应用程序中似乎不能很好地伸缩，所以 React 将应用程序按照其逻辑功能实体进行划分。


The structural units that make up the application's functional entities are React components. A React component defines the HTML for structuring the content, the JavaScript functions for determining functionality, and also the component's styling; all in one place. This is to create individual components that are as independent and reusable as possible.
构成应用程序功能实体的结构单元是 React 组件。 React 组件定义了组织内容的 HTML，确定功能的 JavaScript 函数，以及组件的样式; 所有这些都在一个地方。 这是为了创建尽可能独立和可重用的单个组件。
