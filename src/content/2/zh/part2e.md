...
我们当前应用的外观是相当适度的。 在[练习0.2](/ en / part0 / web apps 基础 # exercises-0-1-0-6)中，作业是浏览 Mozilla 的[ CSS 教程]( https://developer.Mozilla.org/en-us/docs/learn/getting_started_with_the_web/css_basics )。
...
在进入下一部分之前，让我们看看如何向 React 应用添加样式。 有几种不同的方法可以做到这一点，我们将在稍后介绍其他的方法。 首先，我们将以传统的方式将 CSS 添加到我们的应用中; 在一个单独的文件中，不使用[ CSS 预处理器]( https://developer.mozilla.org/en-us/docs/glossary/css_preprocessor )(尽管这并不完全正确，我们将在后面学习)。
...
让我们在样式表中加入如下规则(因为我对优雅网页设计的知识接近于零，所以样式没有多大意义) :
...
使用元素类型来定义 CSS 规则有点问题。 如果我们的应用包含其他<i>li</i> 标记，那么同样的样式规则也应用于它们。
...
React也使得直接在代码中编写样式成为可能，即所谓的[内联样式](inline styles) https://React-cn.github.io/React/tips/inline-styles.html。
...
React的哲学，事实上，是这个极端的对立面。 由于将 CSS、 HTML 和 JavaScript 分离成单独的文件在大型应用中似乎不能很好地伸缩，所以 React 将应用按照其逻辑功能实体进行划分。


The structural units that make up the application's functional entities are React components. A React component defines the HTML for structuring the content, the JavaScript functions for determining functionality, and also the component's styling; all in one place. This is to create individual components that are as independent and reusable as possible.
构成应用功能实体的结构单元是 React 组件。 React 组件定义了组织内容的 HTML，确定功能的 JavaScript 函数，以及组件的样式; 所有这些都在一个地方。 这是为了创建尽可能独立和可重用的单个组件。
...
使用第二章节中的[改进的错误消息](/ en / part2 / add styles to react app # improved-error-message)示例作为指导，显示一个在成功操作执行后持续几秒钟的通知(添加一个人或更改一个数字) :
...
在两个浏览器中打开应用。 * * 如果你在浏览器1中删除一个人 * * 在我尝试在浏览器2中更改该人的电话号码 / i 之前，你会得到如下错误消息:
...
根据第2章节中显示的[ promise and errors ](/ en / part2 / modify data in server # promises-and-errors)中的示例修复问题。 修改此示例，以便在操作不成功时向用户显示消息。 成功和不成功的事件所显示的信息应该看起来不同:
